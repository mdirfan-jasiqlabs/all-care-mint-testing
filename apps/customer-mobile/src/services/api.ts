...
/**
 * Perform a silent token refresh session using the stored refresh token.
 * Rotates both access and refresh tokens when returned by backend.
 */
export const refreshSession = async (): Promise<boolean | 'unauthenticated' | 'offline'> => {
  if (isLoggingOut) {
  ...
  refreshPromise = (async () => {
    try {
      ...
      // Check if user logged out or session was cleared while refresh call was in flight
