...
export class StateEngineService {
  ...
  private readonly transitions: TransitionRule[] = [
    ...
    { from: BookingStatusEnum.PENDING, to: BookingStatusEnum.ASSIGNED, allowedRoles: [ActorRoleEnum.ADMIN] },
    // Provider accepts
    { from: BookingStatusEnum.ASSIGNED, to: BookingStatusEnum.ACCEPTED, allowedRoles: [ActorRoleEnum.PROVIDER] },
    // Provider on the way
    { from: BookingStatusEnum.ACCEPTED, to: BookingStatusEnum.ON_THE_WAY, allowedRoles: [ActorRoleEnum.PROVIDER] },
    // Provider starts
    { from: BookingStatusEnum.ON_THE_WAY, to: BookingStatusEnum.STARTED, allowedRoles: [ActorRoleEnum.PROVIDER] },
    // Provider completes
    { from: BookingStatusEnum.STARTED, to: BookingStatusEnum.COMPLETED, allowedRoles: [ActorRoleEnum.PROVIDER] },
    // Customer/Admin cancels (from PENDING or ASSIGNED)
    { from: BookingStatusEnum.PENDING, to: BookingStatusEnum.CANCELLED, allowedRoles: [ActorRoleEnum.CUSTOMER, ActorRoleEnum.ADMIN] },
    { from: BookingStatusEnum.ASSIGNED, to: BookingStatusEnum.CANCELLED, allowedRoles: [ActorRoleEnum.CUSTOMER, ActorRoleEnum.ADMIN] },
    // Provider rejects (from ASSIGNED, resets to PENDING — note: recorded as REJECTED in history)
    { from: BookingStatusEnum.ASSIGNED, to: BookingStatusEnum.REJECTED, allowedRoles: [ActorRoleEnum.PROVIDER] },
  ];
  ...
  canCancel(currentStatus: BookingStatusEnum): boolean {
    return [BookingStatusEnum.PENDING, BookingStatusEnum.ASSIGNED].includes(currentStatus);
  }

  /**
   * Gets the allowed next statuses for sequential provider updates.
   */
  getNextProviderStatus(currentStatus: BookingStatusEnum): BookingStatusEnum | null {
