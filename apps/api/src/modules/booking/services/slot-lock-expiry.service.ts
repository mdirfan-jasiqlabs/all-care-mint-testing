...
@Injectable()
export class SlotLockExpiryService implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  async runCleanup(): Promise<void> {
    try {
      ...
      if (expiredLocks.length > 0) {
        this.logger.log(`🧹 Found ${expiredLocks.length} expired slot locks. Cleaning up...`);
        for (const lock of expiredLocks) {
          await this.bookingRepo.deleteSlotLock(lock.id);
          this.logger.debug(`🗑️ Released expired lock: ${lock.id} for slot ${lock.slotId} on ${lock.slotDate.toISOString().split('T')[0]}`);
        }
