...
async function runAudit() {
  ...
  try {
    ...
    const resAuthAdmin = await fetch(`${API}/api/v1/auth/admin/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: 'admin@allcaremint.com', password: 'Admin@123' })
    });
    ...
    if (adminToken) {
      ...
      const resSuspendedSend = await fetch(`${API}/api/v1/auth/otp/send`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mobileNumber: '9900000001', role: 'PROVIDER' })
      });
      const bodySuspendedSend = await resSuspendedSend.json();

      const is403Suspended = resSuspendedSend.status === 403 &&
                             bodySuspendedSend.error?.code === 'ERR_PROVIDER_SUSPENDED' &&
                             bodySuspendedSend.error?.message === 'Account suspended.';

      if (is403Suspended) {
        logResult('TC-003-006', 'Suspended Provider OTP Login → 403 ERR_PROVIDER_SUSPENDED', 'PASS', 'Account suspended correctly blocks OTP login', bodySuspendedSend);
      } else {
        logResult('TC-003-006', 'Suspended Provider OTP Login', 'FAIL', `status=${resSuspendedSend.status}, code=${bodySuspendedSend.error?.code}`, bodySuspendedSend);
      }
